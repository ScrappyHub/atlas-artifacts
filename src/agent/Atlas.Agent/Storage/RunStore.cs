...
public sealed class RunStore
{
    ...
    public void InsertRun(string runId, string runType, string createdAt, string outcome, object summary)
    {
        using var conn = Open();
        using var cmd = conn.CreateCommand();
        cmd.CommandText = @""INSERT INTO runs(run_id, run_type, created_at, outcome, summary_json)
VALUES (, , , , );"";
        ...
        cmd.Parameters.AddWithValue("""", JsonSerializer.Serialize(summary, JsonOpts.Serializer));
        cmd.ExecuteNonQuery();
    }
    ...
    public void InsertArtifact(string runId, string artifactKey, string path, string sha256, string createdAt)
    {
        using var conn = Open();
        using var cmd = conn.CreateCommand();
        cmd.CommandText = @""INSERT INTO run_artifacts(run_id, artifact_key, path, sha256, created_at)
VALUES (, , , , );"";
        cmd.Parameters.AddWithValue("""", runId);
        cmd.Parameters.AddWithValue("""", artifactKey);
    ...
    public List<object> GetRunHistory(int limit = 50)
    {
        using var conn = Open();
        using var cmd = conn.CreateCommand();
        cmd.CommandText = @""SELECT run_id, run_type, created_at, outcome
FROM runs ORDER BY created_at DESC LIMIT ;"";
    ...
    public object? GetRunDetails(string runId)
    {
        using var conn = Open();
        using var cmd = conn.CreateCommand();
        cmd.CommandText = @""SELECT run_id, run_type, created_at, outcome, summary_json FROM runs WHERE run_id=;"";
        cmd.Parameters.AddWithValue("""", runId);
        ...
        using var cmd2 = conn.CreateCommand();
        cmd2.CommandText = @""SELECT artifact_key, path, sha256, created_at
FROM run_artifacts WHERE run_id= ORDER BY artifact_key;"";
        cmd2.Parameters.AddWithValue("""", runId);
