...
public sealed class CommandRouter
{
    ...
    public CommandRouter(Atlas.Agent.AppPaths paths, string deviceId, LicenseService license)
    {
        _paths = paths;
        _deviceId = deviceId;
        _license = license;
    ...
    public async Task<IpcResponse> HandleAsync(IpcRequest req)
    {
        ...
        return req.command switch
        {
            ""get_status"" => Ok(req.request_id, new { agent = ""Atlas.Agent"", version = ""0.1.0"", device_id = _deviceId[..12] + ""…’’"" }),
            ""get_license_status"" => Ok(req.request_id, await _license.GetLicenseStatusAsync(_deviceId)),
            ""request_scan"" => await HandleScanAsync(req.request_id),
            ""get_run_history"" => Ok(req.request_id, new { runs = _runs.GetRunHistory(50) }),
            ""get_run_details"" => Ok(req.request_id, new { run = GetRunId(req) is string id ? _runs.GetRunDetails(id) : null }),
            _ => Fail(req.request_id, ""unknown_command"", $""Unknown command: {req.command}"")
        };
    }

    private static string? GetRunId(IpcRequest req)
    {
        if (!req.payload.HasValue) return null;
        if (req.payload.Value.TryGetProperty(""run_id"", out var rid)) return rid.GetString();
        return null;
    }
    ...
    private async Task<IpcResponse> HandleScanAsync(string requestId)
    {
        ...
        try
        {
            ...
            _runs.InsertRun(runId, ""scan_run"", now.ToString(""o""), ""success"", summary);
            ...
            return Ok(requestId, new { run_id = runId, outcome = ""success"", inventory_count = inventory.items.Count, candidate_count = candidates.candidates.Count, run_dir = runDir });
        }
        catch (Exception ex)
        {
            var summary = new { error = ex.Message, stage = ""scan_run"" };
            _runs.InsertRun(runId, ""scan_run"", now.ToString(""o""), ""failed"", summary);
            return Fail(requestId, ""scan_failed"", ex.Message);
        }
    }

    private static IpcResponse Ok(string requestId, object data) => new(requestId, true, null, data);
    private static IpcResponse Fail(string requestId, string code, string message) => new(requestId, false, new IpcError(code, message), null);
}
